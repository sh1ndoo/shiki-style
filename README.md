...
- **Полная поддержка иконок**: Google Material Icons (`[data-icon]`) и Font Awesome 6 (`[data-fa]`, `.fa-brands`, `.fa-solid`)
...
Файл [`css/carousel.css`](css/carousel.css) содержит стили для интерактивной карусели / слайдера изображений в профиле (чистый минималистичный вид без лишних полос снизу, поддержка боковых зон и стрелок переключения, счётчик `data-counter`, режим постеров `data-posters` и тёмный фон `data-dark`).
...
### Большое «Избранное» и компактные «Достижения»
Файл [`css/large-favourites.css`](css/large-favourites.css) меняет компоновку боковой колонки профиля:
- Раздел **«Избранное»** поднимается наверх и разворачивается в полноценную сетку постеров с пропорциями 2:3, мягким зумом при наведении и аккуратными подписями.
- Раздел **«Достижения»** опускается вниз и трансформируется в компактный блок мини-значков (44×44px / 36×36px) без громоздких баннеров.

Подключение через `@import`:
```css
@import "https://raw.githubusercontent.com/sh1ndoo/shiki-style/main/css/large-favourites.css";
```
*(Также доступен алиас `big-favourites.css`)*
...
## Готовый шаблон описания профиля

Красиво сверстанный текст для блока **«О себе»** со всеми вкладками (О стиле, Соцсети, Инструкции по табам и карусели со слайдером) доступен в файле:
👉 **[PROFILE_BBCODE.md](PROFILE_BBCODE.md)**
